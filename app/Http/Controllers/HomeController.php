...
class HomeController extends Controller
{
    public function index(){
        ...
        $data['carousel'] = Business::limit(2)
                                    ->inRandomOrder()
                                    ->get();
        ...
        $data['business_comments'] = Comment::select('users.name as u_name', 'users.id as u_id', 'user_profile.avatar as u_avatar', 'comments.id as comment_id','comments.body', 'comments.score', 'comments.created_at','business.id as business_id', 'business.name as b_name', 'business.score as b_score')
        ...
        $data['products_comments'] = Comment::select('users.name as u_name', 'users.id as u_id', 'user_profile.avatar as u_avatar', 'comments.id as comment_id', 'comments.body', 'comments.score', 'comments.created_at', 'product.id as product_id', 'product.name as p_name', 'product.score as p_score')
                                    ->leftJoin('users', 'users.id', '=', 'comments.user_id')
                                    ->leftJoin('user_profile', 'user_profile.user_id', '=', 'users.id')
