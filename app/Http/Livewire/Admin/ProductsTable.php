...
class ProductsTable extends DataTableComponent
{
    ...
    public function columns(): array
    {
        return [
            Column::make('ID', 'id')
                ->sortable(),
            Column::make('Nombre producto', 'name')
                ->sortable(),
            Column::make('Nombre Negocio', 'business.name')
                ->sortable(),
            Column::make('Categoria', 'categories.name')
                ->sortable(),
            ButtonGroupColumn::make('Acciones')
                ...
                ->buttons([
                    ...
                    ->title(fn($row) => 'Editar')
                    ->location(fn($row) => route('admin_product.edit',['product' => $row->id]) )
                    ...
                    ->title(fn($row) => 'Borrar')
                    ->location(fn($row) => route('admin_product.destroy',['product' => $row->id]) )
                    ->attributes(function($row){
                        return[
