...
class CommentsTable extends DataTableComponent
{
    ...
    public function columns(): array
    {
        return [
            Column::make('ID', 'id')
                ->sortable(),
            Column::make('Comentador', 'user.name')
                ->sortable(),
            Column::make('Comentario', 'body')
                ->sortable()
                ->format(
                    fn($value,$row, Column $column) => "<span class='whitespace-normal'>".$value."</span>"
                )
                ->html(),
            ButtonGroupColumn::make('Acciones')
                ...
                ->buttons([
                    ...
                    ->title(fn($row) => 'Ir al comentario')
                    ->location(fn($row) => route( ( $row->commentable_type == 'App\Models\Product' ? 'product.show' : 'business.show' ),[$row->commentable_id]  ).'#comment_id_'.$row->id )
